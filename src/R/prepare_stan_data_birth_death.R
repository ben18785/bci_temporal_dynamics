# prepares data for birth-death Stan model
...
# combine data ----
## checks
stopifnot(n_distinct(df_short$species) == nrow(counts_offspring))

## combine
data_stan <- list(
  N_generations=ncol(counts_offspring),
  N_species=nrow(counts_offspring),
  firstAppearance=order_df$first_year,
  mutantCounts=mutant_df$x,
  counts_parents=counts_parents,
  counts_offspring=counts_offspring,
