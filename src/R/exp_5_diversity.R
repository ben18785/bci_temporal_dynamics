...
base_filename <- "data/processed/julia_runs/Exp-5_Drift-Reduction/"
file_id <- list.files(base_filename, pattern = "*.csv")
file_id <- map_chr(file_id, ~paste0(base_filename, .))
...
s1 <- a6 %>%
  group_by(censussize,censusyear,simulation_nr, Dt) %>%
  filter(N_present>0) %>%
...
write.csv(s_both, "data/processed/exp_5_diversity.csv")
